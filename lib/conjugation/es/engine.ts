...
function conjugateRegularBase(infinitive: string, group: "ar" | "er" | "ir"): IrregularBase {
  ...
  if (group === "ar") {
    return {
      ...
      pretéritoIndefinido: [stem + "é", stem + "aste", stem + "ó", stem + "amos", stem + "asteis", stem + "aron"],
      subjuntivoPresente: [stem + "e", stem + "es", stem + "e", stem + "emos", stem + "éis", stem + "en"],
      futuroStem: infinitive,
      participio: stem + "ado",
    };
  }
  const presenteEndings = group === "er" ? ["o", "es", "e", "emos", "éis", "en"] : ["o", "es", "e", "imos", "ís", "en"];
  return {
    presente: presenteEndings.map((e) => stem + e) as SixForms,
    pretéritoIndefinido: [stem + "í", stem + "iste", stem + "ió", stem + "imos", stem + "isteis", stem + "ieron"],
    subjuntivoPresente: [stem + "a", stem + "as", stem + "a", stem + "amos", stem + "áis", stem + "an"],
    futuroStem: infinitive,
    participio: stem + "ido",
  };
}
...
function deriveImperfecto(infinitive: string, override?: SixForms): SixForms {
  ...
  const stem = infinitive.slice(0, -2);
  return infinitive.endsWith("ar") ? withEndings(stem, IMPERFECTO_AR_ENDINGS) : withEndings(stem, IMPERFECTO_ER_IR_ENDINGS);
}

export function buildConjugation(verb: VerbEntry): ConjugationTable {
  const base: IrregularBase = verb.kind === "regular" ? conjugateRegularBase(verb.infinitive, verb.group) : verb;
