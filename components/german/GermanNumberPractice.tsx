...
// 逆引きルックアップ用（ウムラウト文字から基本文字グループのキーを引くためのオブジェクト）
...
/**
 * 上下キー操作により、指定された文字をウムラウトバリエーション内で順次サイクル切り替えする関数。
 * @param char 対象の文字
 ...
 */
function filterItemsByRange(items: GermanNumberItem[], range: RangePreset): GermanNumberItem[] {
...
export default function GermanNumberPractice({ items }: { items: GermanNumberItem[] }) {
  ...
  const buildQuestions = useCallback(
    (itemsList: GermanNumberItem[], range: RangePreset, count: CountOption) => {
  ...
  // 初期状態は仮の配列（マウント後に決定）
  const [questions, setQuestions] = useState<GermanNumberItem[]>([]);
  ...
  const handleSubmit = useCallback(() => {
    ...
    const normalizedTarget = currentQuestion.german.trim().toLowerCase();
  ...
  /**
   * 入力欄でのキーボード入力ハンドラ。
   * ↑ / ↓ 矢印キーで文字のウムラウト・エスツェット切り替えを行い、Enterキーで解答送信する。
   */
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === "ArrowUp" || e.key === "ArrowDown") {
      ...
      // 変換対象文字が存在する場合、ウムラウト・エスツェットを切り替えて更新
      if (targetIdx !== -1) {
          ...
          <div className="mt-8 flex flex-col items-center">
            <div className="w-full max-w-lg">
              ...
              <p className="mt-3 text-center text-xs md:text-sm text-zinc-500 dark:text-zinc-400 leading-relaxed">
                💡 <strong>↑ / ↓ 矢印キー</strong> で文字にウムラウト・エスツェット（ä, ö, ü, ß
                など）を付与・切替できます。
              </p>
            </div>
