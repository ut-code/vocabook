    ...
    <header className="sticky top-0 z-50 bg-white/5 dark:bg-zinc-600/80 backdrop-blur-md border-b border-black/[.08] dark:border-white/[.145]">
      <div className="mx-auto flex w-full max-w-5xl items-center justify-between px-6 py-4">
        <Link href="/" className="flex item-center gap-3">
          ...
          />
          <span className="text-lg font-semibold tracking-tight self-center hover:text-emerald-500 dark:hover:text-emerald-500">
          Vocabook
          </span>
        </Link>
        <nav className="flex gap-6 text-sm font-medium text-zinc-600 dark:text-zinc-200">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
