...
// セル1つ分の表示。値が1件なら普通のテキスト、複数件なら番号付きリストで
// セル内に積んで表示する（列同士の行は揃えない。この単語帳が持つセルはそれぞれ独立）
function CellValues({ values }: { values: string[] }) {
...
export default function CardRow({
  ...
  if (!editing) {
    // 見出し語1件につき常に1行。各セルは値が複数あればセル内で積んで表示する
    // （列ごとの件数は完全に独立しているため、行を分けて揃える必要がない）
    return (
      <tr className="border-t border-black/[.06] dark:border-white/[.1]">
        ...
        {bodyColumns.map((column) => (
          ...
          >
            <CellValues values={card.data.cells[column] ?? []} />
          </td>
        ))}
    ...
    <tr className="border-t border-black/[.06] dark:border-white/[.1]">
      <td colSpan={bodyColumns.length + 2} className="px-4 py-3">
        ...
        <form action={formAction} className="flex flex-col items-start gap-3">
          {/* defaultHead/defaultCellsで現在の値を初期表示し、そこから編集する */}
          <CardFieldsForm columns={columns} defaultHead={card.data.head} defaultCells={card.data.cells} />
